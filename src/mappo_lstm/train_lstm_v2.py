...
class TransformerTrajectoryPredictor(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, num_heads, num_agents):
    ...
    def forward(self, x):
        # x: [seq_len, num_agents, input_dim]
        # Transformer 输入需要是：[seq_len, batch_size, input_dim]
        
        x = x.permute(1, 0, 2)  # 调整维度：[num_agents, seq_len, input_dim] -> [seq_len, num_agents, input_dim]
        
        # 通过 Transformer 编码器处理
        x = self.transformer_encoder(x)

        # 预测未来轨迹
        pred = self.fc(x)  # [seq_len, num_agents, output_dim]
        return pred

# 策略网络(Actor)，增加Transformer 轨迹预测模块
class PolicyNet(torch.nn.Module):
    def __init__(self, state_dim, hidden_dim, action_dim, team_size, num_heads):
        ...
        # Transformer 轨迹预测模块
        self.trajectory_predictor = TransformerTrajectoryPredictor(input_dim=hidden_dim, 
                                                                   hidden_dim=hidden_dim, 
                                                                   output_dim=team_size * state_dim * 3,
                                                                   num_heads=num_heads,
    ...
    def forward(self, x):
        ...
        # 轨迹预测
        trajectory_pred = self.trajectory_predictor(x.unsqueeze(0))  # [1, team_size, hidden_dim]
        return action_probs, trajectory_pred.squeeze(0)  # [team_size * state_dim * 3]
...
class MAPPO:
    def __init__(self, team_size, state_dim, hidden_dim, action_dim, actor_lr, critic_lr, lmbda, eps, gamma, device, num_heads):
        self.team_size = team_size
        self.gamma = gamma
        self.lmbda = lmbda
        self.eps = eps
        ...
        # 为每个智能体创建独立的actor，并传入num_heads用于Transformer
        self.actors = [PolicyNet(state_dim, hidden_dim, action_dim, team_size, num_heads).to(device) 
    ...
    def update(self, transition_dicts, state_dim):
        ...
        states_all = torch.tensor(np.array(states_all), dtype=torch.float).to(self.device)
        next_states_all = torch.tensor(np.array(next_states_all), dtype=torch.float).to(self.device)

        rewards_all = torch.tensor([[transition_dicts[i]['rewards'][t] for i in range(self.team_size)]
                                     for t in range(T)], dtype=torch.float).to(self.device)
        ...
        for i in range(self.team_size):
            ...
            current_probs, pred_out = self.actors[i](states)
            ...
            for t in range(T-2):
                if t+2 < T:
                    traj = torch.cat([next_states_all[t],
                                      next_states_all[t+1],
                                      next_states_all[t+2]], dim=-1)
                    target_list.append(traj)
            if target_list:
                target_tensor = torch.stack(target_list)
                pred_subset = pred_out[:target_tensor.size(0)]
...
mappo = MAPPO(team_size, state_dim, hidden_dim, action_dim, actor_lr, critic_lr,
              lmbda, eps, gamma, device, num_heads=4)
